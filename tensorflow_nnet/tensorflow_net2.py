...
def db2np(db_trans, limit=None, skip=0):
    ...
    for one_image in cursor:
        ...
        X[i,:,:,0] = img_array.reshape(IMAGE_SIZE, IMAGE_SIZE)/np.float32(255)-.5
...
def load_dataset(limit=None, skip=0):
    ...
    sss = cross_validation.StratifiedShuffleSplit(y[:,1], n_iter=1, test_size=VALIDATION_SIZE, random_state=3476)
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  def model(data, train=False):
    ...
    if train:
      pool = tf.nn.dropout(pool, 0.5, seed=SEED)
    conv = tf.nn.conv2d(pool,
                        conv3_weights,
                        strides=[1, 1, 1, 1],
                        padding='VALID')
